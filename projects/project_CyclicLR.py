...
MyModel = __import__("SATELLITE-IMAGE-CLASSIFICATION.models.MyModel")#这里切换模型
...
SCHEDULER = 'CyclicLR'
scheduler = optim.lr_scheduler.CyclicLR(optimizer, base_lr=0.0000002, max_lr=LEARNING_RATE,cycle_momentum=False,step_size_up = len(train_data) // BATCH_SIZE * 5)
print(f"step_size_up:{len(train_data) // BATCH_SIZE * 5}")
##############################################################################   训练
print("Training...")
...
plt.savefig(f"figs/{project_name}_{model.name}--{BATCH_SIZE}_{num_epochs}_{SCHEDULER}.png")
plt.show()
###################

plt.figure()
plt.plot(x_axis,learning_rate)
plt.savefig(f"../figs/{project_name}_{model.name}--{BATCH_SIZE}_{num_epochs}_{SCHEDULER}_RATE.png")
plt.show()



with open(f"../figs/{project_name}_{model.name}--{BATCH_SIZE}_{num_epochs}_{SCHEDULER}.csv","w") as f:
    f.write("epoch,accuracy\n")
    for i in range(len(x_axis)):
        f.write(f"{x_axis[i]},{acc[i]}\n")

with open(f"../figs/{project_name}_{model.name}--{BATCH_SIZE}_{num_epochs}_{SCHEDULER}_RATE.csv","w") as f:
    f.write("epoch,learning_rate\n")
    for i in range(len(x_axis)):
        f.write(f"{x_axis[i]},{learning_rate[i]}\n")

print("Saving model...")
torch.save(model, f'../model_outputs/{project_name}_{model.name}--{BATCH_SIZE}_{num_epochs}_{SCHEDULER}.pth')
print("Finish")
